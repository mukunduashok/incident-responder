...
class GitSearchInput(BaseModel):
    ...
    timestamp: str = Field(..., description="Timestamp to search commits before (ISO format)")
    max_commits: int = Field(default=5, description="Maximum number of commits to retrieve")
...
class GitSearchTool(BaseTool):
        ...
        try:
            ...
            for line in result.stdout.strip().split('\n'):
                if line:
                    ...
                    files_cmd = [
                        "git", "-C", str(repo_path),
                        "show", "--name-only", "--pretty=format:", hash_val
                    ]
            ...
            for idx, commit in enumerate(commits, 1):
                ...
                if len(commit['files_changed']) > 5:
                    output.append(f"     ... and {len(commit['files_changed']) - 5} more")
    ...
    def _assess_risk(self, files: list, message: str) -> str:
        ...
        """
        high_risk_patterns = [
            'migration', 'database', 'schema', '.sql',
            'config.yaml', 'config.json', 'settings',
            'requirements.txt', 'package.json', 'dependencies'
        ]
        
        medium_risk_patterns = [
            'api', 'endpoint', 'route', 'handler',
            'service.py', 'controller', 'middleware'
        ]
        ...
        if any(word in message_lower for word in ['critical', 'hotfix', 'urgent', 'breaking']):
            return "HIGH"
